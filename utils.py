...
def get_shapes_dict(dataset_path):
    ...
    for name in sorted_dataset_names:
        shapes_dict[name] = (
            int(datasets_df.set_index("names").loc[name]["num_cells"]),
            8000,
        )
...
def figshare_download(url, save_path):
    ...
    if os.path.exists(save_path):
        ...
        )  # NOTE: might need headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36'})
        block_size = 1024
