...
class MultiDatasetSentences(data.Dataset):
    def __init__(
        ...
        self.dataset_to_protein_embeddings = torch.load(
            dataset_to_protein_embeddings_path
        )
        ...
        if isinstance(idx, int):
            for dataset in sorted(self.datasets):
                if idx < self.num_cells[dataset]:
                    ...
                    if torch.sum(weights) > 0:  # we filter for these later
...
def sample_cell_sentences(
    ...
    for c, cell in enumerate(counts):
        ...
        choice_idx = np.random.choice(
            np.arange(len(weights)), size=args.sample_size, p=weights, replace=True
        )
        ...
        ordered_choice_idx = np.full(
            (args.pad_length), args.cls_token_idx
        )  # start with cls
